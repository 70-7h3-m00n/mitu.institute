...
const webhook = async (
  ...
  if (req.body) {
    ...
    try {
        ...
        if(isAffiliateFromAmocrm && isEdpartnersFromAmocrm && price) await axios.get(`https://offers-edpartnersaffise.com/postback?secure=${AFFISE_SECURE}&clickid=${``}&order_sum=${price}&goal=1`)

        const utm_source   = req?.body?.utms?.utm_source
        const utm_campaign = req?.body?.utms?.utm_campaign
        const cl_uid = req?.body?.utms?.cl_uid
        if (utm_source === 'edpartners' && utm_campaign === 'affiliate' && !price) await axios.get(`https://offers-edpartnersaffise.com/postback?secure=${AFFISE_SECURE}&goal=1&clickid=${cl_uid}&comment=offer&name=${name}&phone=${phone}&email=${email}`)
